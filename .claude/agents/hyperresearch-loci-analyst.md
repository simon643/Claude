...
1. **Load the corpus.** Use `/tmp/hyperresearch/.venv/bin/hyperresearch note list --tag <corpus_tag> --all --json`
...
2. **Read breadth first.** For each note, read the title + summary + first
   ~400 chars (use `/tmp/hyperresearch/.venv/bin/hyperresearch note show <id> -j` and truncate). Do NOT read
